...
export const useStepNavigation = () => {
  ...
  const getStepIndex = (step: Step): number => {
    const steps: Step[] = ['personType', 'basicInfo', 'contactInfo', 'success'];
    return steps.indexOf(step);
  };

  const handleNext = () => {
    const steps: Step[] = ['personType', 'basicInfo', 'contactInfo'];
  ...
  const setPersonTypeAndNext = (selectedPersonType: PersonType) => {
    if (
      selectedPersonType === PERSON_TYPE.NATURAL ||
      selectedPersonType === PERSON_TYPE.JURIDICAL
    ) {
  ...
  // Computed values
  const isLastStep = currentStep === 'contactInfo';
